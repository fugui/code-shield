...
// Global mutex to serialize all Outlook OLE COM automation calls.
// Outlook is a single-instance STA application and cannot handle concurrent COM thread requests.
var outlookMutex sync.Mutex
...
func BatchSendOutlookDrafts() (int, error) {
	...
	for i := count; i >= 1; i-- {
		...
		if err == nil && classProp != nil && int(classProp.Val) == 43 {
			subjectProp, err := oleutil.GetProperty(mailItem, "Subject")
			if err == nil && subjectProp != nil {
