...
| **三级指纹分层兜底 (Layered Fingerprint)** | 构建 **L1 (精确源码行强指纹) $\to$ L2 (函数作用域弱指纹) $\to$ L3 (物理行号区间兜底指纹)** 的三级阶梯匹配体系。 | 即使开发者重构代码、调整行号或小幅修改，依然能稳定继承历史生命周期 |
...
flowchart TB
    ...
    subgraph Phase3["3. 三级确定性指纹计算 (Layered Fingerprint Generator)"]
        RealTrigger & RealScope --> L1Gen["L1 强指纹: SHA256(Repo, Task, Path, ScopeSymbol, RealTrigger)"]
        RealScope & NormCategory --> L2Gen["L2 作用域弱指纹: SHA256(Repo, Task, Path, ScopeSymbol, NormCategory)"]
        RealTrigger --> L3Gen["L3 物理位置兜底指纹: SHA256(Repo, Task, Path, LineWindow, NormCategory)"]
    end

    subgraph Phase4["4. 抗抖动平滑增量状态机 (Robust Diff Engine)"]
        L1Gen & L2Gen & L3Gen --> DiffEngine["Diff & Life-Cycle Engine"]
        DiffEngine -->|三级级联匹配命中| ExistedFlow["标记 EXISTED / REOPENED\n(重置 missed_count = 0, 更新活跃时间)"]
        DiffEngine -->|三级均未命中| NewFlow["标记 NEW\n(落库创建指纹档案)"]
...
func DiffAndEnrichFindings(repoID uint, taskReportID uint, taskTypeID uint, repoRoot string, scannedFiles []string, findings []models.AnalysisFinding) ([]models.AnalysisFinding, error) {
	...
	for i := range findings {
		...
		if matchedRecord != nil {
			...
			newRec := models.DefectFingerprintRecord{
				RepoID:             repoID,
				...
				Status:             "ACTIVE",
				MissedCount:        0,
				FileHashSnapshot:   fileContentHashMap[strings.ToLower(f.FilePath)],
				FirstTaskID:        taskReportID,
				LastTaskID:         taskReportID,
				FirstSeenAt:        now,
				LastSeenAt:         now,
			}
			models.DB.Create(&newRec)
		}
		enrichedList = append(enrichedList, *f)
	}
	
	// 4. 【抗抖动核心】已扫描文件中未命中历史缺陷的生命周期决策
...
### 4.5 模型固化与 Schema 契约边界

#### 4.5.1 职责边界划分契约
| 实体 | 负责范畴 (SSOT) | 绝对禁止跨界的范畴 |
| :--- | :--- | :--- |
| **大模型 (LLM)** | • 发现潜在代码缺陷<br>• 输出精确位置坐标 (`file_path`, `line_number`)<br>• 生成解释性文本 (`title`, `detail`, `suggestion`) | • 严禁计算或输出指纹 Hash<br>• 严禁决定缺陷生命周期状态 (`status`)<br>• 严禁输出非受控分类命名 |
| **CodeShield 服务端** | • 根据物理文件提取精确 `TriggerLine`<br>• AST 全文件回溯提取 `ScopeSymbol`<br>• 白名单字典归一化 `Category`<br>• 计算 L1/L2/L3 指纹与增量状态机打标 | • 严禁盲目信任 LLM 自由文本作为唯一标识<br>• 严禁在代码无物理变更时单次标 `RESOLVED` |
...
flowchart LR
    ...
    T5["Test 5: 真实修复状态流转测试\n(代码物理修改且连续未命中，断言正确置为 RESOLVED)"]
```
...
gantt
    ...
    section 阶段三：数据模型迁移与全量回归测试
    数据库 DefectFingerprintRecord 自动迁移 :2026-09-09, 1d
    编写全套抗漂移与状态机单测 (fingerprint_test / diff_engine_test) :2026-09-10, 2d
    全仓端到端双跑回归验证与发布 :2026-09-12, 1d
```
...
本设计通过**“物理源码提取确定性特征”**替代**“脆弱的大模型生成文本”**，通过**“受控分类枚举与映射”**规避**“分类自由词发散”**，通过**“文件内容变更守卫与连续未命中计数”**终结**“单次模型抖动导致的假修复大面积震荡”**。
