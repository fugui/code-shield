...
- **P8 任务类型模式自适应原则（Task-Adaptive Governance Mode）**：不同检视任务具有本质不同的业务使命，绝不能“一刀切”套用同一种对账处理逻辑：
  1. **全量基线检视（如 `deep_review`）**：执行**全量台账模式（Full Ledger Mode）**，面向整仓全局质量大盘，维护跨轮完整 SSOT 台账，管理长周期覆盖缺口与退火；
  2. **近期变更检视（如 `change_review` / PR 门禁）**：执行**变更增量焦点模式（Change Delta Focus Mode）**。**严格实行“历史无关问题旁路隔离”**——未被本次变更触碰的文件与历史存量问题**绝对不拉取、不处理、不合入本报告**；双向聚焦于“本次改动新引入缺陷（NEW_IN_DIFF）”与“本次改动是否修复了触碰的历史缺陷（PROOF_OF_FIX）”，彻底杜绝陈年旧账刷屏干扰变更评审；
  3. **专项领域检视（如 `coredump_risk`、`memory_leak`）**：执行**专项领域穿透模式（Domain Specialized Mode）**，在独立 TaskType 命名空间内闭环治理，不与全量检视交叉污染。
...
在任务配置（`tasks/*/meta.json` 及 `TaskType` 模型）中显式声明 `governance_mode`，对账引擎根据该枚举自适应路由到对应的处理管线：

| 维度 | **模式 A：全量基线台账模式 (`full_ledger`)** | **模式 B：变更增量焦点模式 (`change_focus`)** | **模式 C：专项领域穿透模式 (`domain_specialized`)** |
| :--- | :--- | :--- | :--- |
| **代表任务** | `deep_review`（全量深度检视）、全量合规审查 | `change_review`（近期变更检视）、PR / MR 门禁 | `coredump_risk`、`memory_leak`、`cjson_scan` 等 |
| **扫描范围** | 全仓全量文件（Full Repo） | **仅近期提交变动文件与 Diff Hunks（`since_days` > 0 或 PR Diff）** | 全仓中命中特定语法规则的特征文件 |
| **历史问题策略** | **全量维护**：继承历史台账，追踪覆盖缺口，维护完整 SSOT | **完全旁路隔离（Bypass）**：未被本次变更触碰的历史问题**不读取、不处理、不合入** | **领域内独立追踪**：仅维护属于该专项 TaskTypeID 的指纹空间 |
| **VANISHED 处理** | 代码未动 = `COVERAGE_GAP`；代码变动 = 观察修复候选 | **绝不产生 `COVERAGE_GAP`**！不在本次变动范围内的历史问题直接忽略 | 仅在专项规则关注的函数/代码块内判定漏报或修复 |
| **历史修复判定** | 双层物理守卫 + 连续两轮未命中平滑观察期 | **顺带核销（Proof of Fix 反哺）**：变动行若正好触碰历史锚点且隐患消失，顺带核销并反哺全量库 | 专项缺陷对应代码行重构核销 |
| **产物输出形式** | 完整问题台账 `synthesis.json`（含存量、缺口、冷归档）+ 全量 Markdown 报告 | **纯变更焦点报告**（仅含“本次变更引入新缺陷”与“顺带修复历史缺陷”） | 专项指标卡片与垂直领域缺陷明细 |
...
flowchart TD
    ...
    subgraph ScopeGuard["2. 历史问题旁路隔离 (Bypass Guard)"]
        ...
        FILTER -->|未触碰文件/行| BYPASS["完全旁路豁免 (不读取、不处理、绝不合入本次报告)"]
    ...
    subgraph DualFocus["3. 双向聚焦核心判定"]
        ...
        TOUCHED --> CHECK_NEW
        CHECK_NEW -->|否 (全新位置/全新逻辑)| NEW_DIFF["NEW_IN_DIFF (本次引入新缺陷，CI 拦截卡点)"]
...
type ScanReconciliation struct {
	...
	// 任务模式与变更焦点 (Phase 3.7)
	GovernanceMode        string `gorm:"size:32;default:'full_ledger';index" json:"governance_mode"` // full_ledger / change_focus / domain_specialized
...
├── 2. 实现 §3.7 治理模式路由器：根据 TaskType.GovernanceMode 路由至 full_ledger / change_focus / domain_specialized
...
| 现有代码/文件 | 本文动作 |
| :--- | :--- |
| `tasks/*/meta.json` 与 `models.TaskType` | 新增 `governance_mode` 属性枚举（`full_ledger` / `change_focus` / `domain_specialized`），默认 `full_ledger`；`change-review` 设为 `change_focus` |
