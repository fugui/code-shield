...
flowchart TB
    ...
    subgraph Phase4["Phase 4: 持久化与产物"]
        SAME & NEW & REL & TEMPLATE & GAP & FIX --> SYN["修订后的 synthesis-{B}.json（完整问题台账 SSOT）\n(标历史重复 + 补入未复现 + 新增条目 + 观察历史)"]
...
type ReconciliationLink struct {
	...
	BaseItemIDX     uint `gorm:"index" json:"base_item_idx"`    // A 数组下标 (合成 JSON 无稳定 id，用下标)
	CurrentItemIDX  uint `gorm:"index" json:"current_item_idx"` // B 数组下标
	...
	Relation    string  `gorm:"size:40;index" json:"relation"` // SAME / SAME_MULTI_VIEW / PROBABLE / SPLIT_FROM / MERGED_INTO / TEMPLATE / VANISHED_COVERAGE_GAP / VANISHED_FIX_CANDIDATE / NEW
...
台账每条 = 一个**实体**（物理锚点为身份），并携带**观察历史**。新增/修订字段：

| 字段 | 说明 |
| :--- | :--- |
| `fingerprint` | 物理锚点 L1 强指纹（稳定身份，跨轮不变） |
| `diff_status` | 本轮视角：`NEW` / `EXISTED`(历史发现) / `REOPENED` |
| `lifecycle_status` | 台账视角：`ACTIVE` / `COVERAGE_GAP`(本轮未复现、代码未变) / `VERIFIED_PENDING` / `RESOLVED` |
| `rounds_seen` | 依次记录在哪些报告轮次中被发现（如 `[19371, 19375]`）；"本轮是否出现" = `rounds_seen[-1] == 本轮` |
| `first_seen_report` / `last_seen_report` | 首次 / 最近检出轮次 |
| `missed_count` | 物理代码已变更时的连续未命中计数（修复确认观察期） |
| `coverage_gap` | 布尔：本轮未复现但因代码未变而保留的状态 |
| `template_family_id` | 跨组件同模板聚簇 ID（不合并实体） |
| `recon_relation` / `recon_base_idx` | 本轮对账关系（SAME / MULTI_VIEW / SPLIT / TEMPLATE）与基线条目下标 |
| `payload` | 最新一次观测的认知载荷（title/detail/severity/category/trigger_line/code_snippet 等） |

#### 5.1.2 什么是"完整清单"

对账后 `report-{B}-synthesis-{repo}.json` = **B 全部条目（打上 diff_status）+ A 中本轮未复现条目（补入，标 COVERAGE_GAP）+ 归一化去重（MULTI_VIEW 合并、TEMPLATE 聚簇）**：
...
```json
{
  ...
  "items": [
    {
      "fingerprint": "12f5...", "diff_status": "NEW", "lifecycle_status": "ACTIVE",
      ...
      "coverage_gap": false, "template_family_id": "",
      "payload": {"title": "GetMMDC values[i] 无下标保护", "severity": "建议", "category": "逻辑与正确性-边界处理"}
    },
    {
      "fingerprint": "bcf6...", "diff_status": "EXISTED", "lifecycle_status": "ACTIVE",
...
#### 5.1.3 派生视图（保证本轮原始发现不丢）

- **本轮新增**：`diff_status == NEW`
- **本轮确认存量**：`diff_status == EXISTED && rounds_seen[-1] == 本轮`
- **本轮未复现**：`rounds_seen[-1] != 本轮`
- **报告 / LLM 输入**：取 `rounds_seen[-1] == 本轮` 的子集 + 增量统计（synthesis-AI-input 由它派生）
- **下一轮对账基线**：直接以本台账文件为基线（不再依赖历史 DB）
...
| B 未复现且仓库未变（如 build.sh 注入、SfmErrorCodes.h 等 ~50 条） | `VANISHED_COVERAGE_GAP` | **严禁 RESOLVED** |
...
├── 2. 复用 SourceEnricher / TaxonomyNormalizer / fingerprint.go 计算每条款锚点特征
└── 3. models 新增 ScanReconciliation / ReconciliationLink，AutoMigrate
...
Phase 4: VANISHED 守卫与修复判定 (1 天)
├── 1. repo_unchanged 判定（commit 或文件内容 Hash）
├── 2. Git diff Hunk 触碰判定（复用 04 §4.2 L1/L2 守卫）
└── 3. 硬断言：repo_unchanged 时 RESOLVED_FROM_VANISHED == 0

Phase 5: 持久化写回与基线播种 (1 天)
├── 1. DiffStatus 写回 AnalysisFinding；ScanReconciliation 汇总统计
├── 2. 可选：以基线报告播种 DefectFingerprintRecord（对齐 04 §4.6 无损迁移）
└── 3. 幂等（同一 (A,B) 重跑不产生重复链接，支持 --dry-run）

Phase 6: 产物与集成 + 金标回归 (1~1.5 天)
├── 1. recon-*.json 数据工件生成器 + 现有报告的增量渲染（增量概要/徽标/跨轮结论）
├── 2. exporter_reconciliation.go + handler + 前端增量视图
└── 3. reconcile_golden_test.go（含 §七 8 项断言）全绿后发布
```
...
| 覆盖能力 | Tier1/Tier2 / 双层守卫 / Proof of Fix（04） | R1~R6 六层漏斗 + 拆并/模板族/多视角/VANISHED 分类 |
...
| `services/task_runner.go`（executeSynthesis 阶段） | 把对账结果（`diff_status` / recon 链接 / family / 未复现补入）**修订写回 synthesis 为完整台账**，并由台账派生 synthesis-AI-input 增量视图 |
| `services/reports/exporter_markdown.go` | 渲染"增量概要 / 条目徽标 / 跨轮治理结论"，替代原先设想的独立 recon 报告 |
