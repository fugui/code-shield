...
│ 条件: 同 Path + 同 NormalizedScope                          │
│      且 (1) 强空间邻近: 行号位移 ≤ 3 行 或 Token Jaccard > 0.75│
│          (此时即便 Category 略微漂移，亦判定同一缺陷并自愈)    │
...
```go
// MatchFindingInFunnel 漏斗比对核心实现（含强空间优先与跨分类自愈对齐）
func MatchFindingInFunnel(
	...
	for _, cand := range candidates {
		...
		// 判定准则 A【强空间邻近优先】：同一函数内，物理行极其接近 (≤ 3 行) 或 Token 高度相似 (> 0.75)
		// 彻底解决 03 报告中 Match 7/10 因大模型将“越界”误标为“竞争/异常”导致的分类阻断死锁
		hitStrongSpatial := (lineDiff <= 3 || tokenSim > 0.75)

		// 判定准则 B【同分类局部容错】：分类一致时，允许更大的局部探索窗口 (≤ 15 行 或 Jaccard > 0.65)
		hitSameCatSpatial := isSameCategory && (lineDiff <= 15 || tokenSim > 0.65)

		if hitStrongSpatial || hitSameCatSpatial {
			log.Printf("[Tier2 Match] Scope clustering hit: Defect #%d (line %d, cat=%s) matched new finding (line %d, cat=%s, sim=%.2f)\n",
...
```
Phase 1: 物理锚点提取器与双层物理守卫落地 (解决“无故关闭”与“行号漂移” - 预计 1 天)
├── 1. 实现 SourceEnricher:
...
└── 3. 验证: 代码未改动或仅改动无关函数时二次全仓扫描，断言 RESOLVED 数量严格为 0

Phase 2: 指纹公式解耦与四级漏斗重构 (解决“重复报新”与“分类锁死” - 预计 1.5 天)
├── 1. 重构 fingerprint.go: L1 强指纹绑定 (Path + NormalizedScope + PhysToken)，彻底剔除自然语言与 Category
├── 2. 升级 diff_engine.go: 实现 Tier 1 (L1强指纹) 与 Tier 2 (强空间优先与跨分类自愈对齐)
├── 3. 加入 Tier 2 自愈时的 DB 唯一索引防冲突与合并逻辑
└── 4. 验证: 用现网《03》报告中的 22 组实测异动案例进行单测回归，断言对齐率 100%
...
├── 3. 实现全仓待扫文件清单确定性排序切块，消灭 35% 调度覆盖漏扫
└── 4. 全仓双跑端到端验收与发布上线
```
