...
> 🛡️ **铁律**：**代码未动，缺陷永不自动关闭！**
> 缺陷流转为 `RESOLVED` 的充要条件是：**“承载该缺陷的代码行或所在函数，在物理世界中确实被修改或删除了”**。
> 如果物理文件的 Git Blob Hash 未发生任何改变，无论大模型在单次或多次扫描中是否提及该缺陷，系统强制将其锁定在 `ACTIVE` 状态，并将其标记为“大模型单次漏报”，严禁关闭。

### 基石二：双轨制解耦架构 (Identity & Payload Decoupling)
> 🧬 **铁律**：**物理锚点决定唯一身份，认知载荷决定展示体验。**
> - **物理锚点 (Physical Anchor - SSOT)**：仅由 `(RepoID, TaskTypeID, NormalizedPath, EnclosingScope, PhysicalTriggerToken)` 决定。纯物理客观特征，永不受大模型自然语言影响。
...
> 当文件被正常修改时（例如文件前部增加了 100 行代码导致行号整体下移，或者大模型在同一函数内把触发行选偏了 2 行），不依赖生硬的字符全等哈希，而是通过**物理行 Token 对齐 $\rightarrow$ 函数空间单体聚类 $\rightarrow$ Git Diff 变更块映射 $\rightarrow$ 语义自愈**的四级漏斗，确保代码演进时缺陷生命周期平滑继承。
...
flowchart TB
    ...
    subgraph GroundingPhase["2. 物理源码真实源感知器 (Source-Grounded Anchor Enricher)"]
        ...
        SrcEnricher -->|物理读取指定行真实代码并去噪| PhysTrigger["物理单行 Token (CleanPhysicalTrigger)"]
        SrcEnricher -->|全文件逆向 AST / 正则回溯| RealScope["物理核心作用域 Class::Method (EnclosingScope)"]
        SrcEnricher -->|基于 trigger 全文反查| ExactLine["真实精准行号 (start_line - end_line)"]
    ...
    subgraph FunnelPhase["3. 四级空间几何比对漏斗 (The 4-Tier Spatial Engine)"]
        ...
        T1 -->|❌ 未命中| T2{"Tier 2: 作用域单体聚类与距离容错\n(同 Path + 同 Scope + 同 Category\n且行号偏移 ≤ 15 行 或 Token Jaccard > 0.65)"}
    ...
    subgraph LifecyclePhase["4. 防假修复与三相确认状态机 (Robust Lifecycle Guard)"]
        MatchSuccess & HealSuccess --> KeepActive["重置 missed_count = 0\n更新活跃时间与最近任务 ID"]
        
        HistoryRecords["历史已入库 ACTIVE 缺陷记录"] --> DiffGuard{"检查未被本次扫描命中的历史缺陷"}
        
        DiffGuard --> CheckHash{"当前物理文件 Hash 与上次检出 Hash 比较"}
        CheckHash -->|物理 Hash 一模一样 (代码未变!)| SuppressGuard["【🚨 拦截假修复】\n判定为 LLM 概率漏报\n强制保持 ACTIVE，严禁关闭!"]
        
        CheckHash -->|物理 Hash 已改变 (代码确实被改动)| EvalMissed{"连续未命中次数 missed_count + 1\n是否达到确认阈值 N (默认 2)?"}
        EvalMissed -->|未达阈值 (N < 2)| GracePeriod["【观察期保持 ACTIVE】\n记入 missed_count，暂不关闭"]
        EvalMissed -->|达到阈值 (N >= 2) 且代码行已删除| RealResolve["【正式标记已修复 RESOLVED】\n代码真实修改且多轮验证确认消失"]
    end
```
...
type SourceAnchor struct {
	...
	EnclosingScope string // AST/逆向回溯提取的物理函数/方法签名 (e.g. "BufferPool::Allocate")
	PhysicalToken  string // 从物理文件中读取并清洗的单行真实核心代码语句
	StartLine      int    // 物理文件中的真实绝对起始行
	EndLine        int    // 物理文件中的真实绝对结束行
}

// EnrichSourceAnchor 从磁盘物理源码中计算确定性特征
func EnrichSourceAnchor(repoRoot string, filePath string, rawLine string, rawTrigger string) (*SourceAnchor, error) {
	...
	if targetLine <= 0 || targetLine > totalLines {
		// 行号缺失或越界，利用 trigger_line 在物理文件中全文模糊定位行号
		targetLine = locateTriggerInLines(lines, rawTrigger)
	}
	if targetLine <= 0 {
		targetLine = 1
	}

	// 2. 从物理文件目标行直接提取真实代码，进行 Token 级清洗 (去注释、去空白、去单双引号、去分号)
	physicalLine := lines[targetLine-1]
	cleanPhysicalToken := cleanSourceToken(physicalLine)
	if cleanPhysicalToken == "" && targetLine < totalLines {
		// 若该行恰好为空或注释行，顺延读取下一有效物理行
		cleanPhysicalToken = cleanSourceToken(lines[targetLine])
	}

	// 3. 以全文件为上下文，从 targetLine 向上逆向扫描，提取最内层 enclosing function/class
	scopeSymbol := ExtractScopeSymbolFromFullContent(normPath, lines, targetLine)

	return &SourceAnchor{
		NormalizedPath: normPath,
		EnclosingScope: scopeSymbol,
...
#### 关键优势：
* **大模型怎么胡说八道都不怕**：大模型即使在 `trigger_line` 里填入了一大段带缩进、换行、带省略号的自由文本，系统也会从磁盘对应行读取真实的物理代码，**指纹特征的来源 100% 客观可复现**。
* **空行号 100% 自动自愈**：利用 `locateTriggerInLines`，现网 R2 出现的 20 处空行号在入库前全部会被自动纠正为磁盘源码上的真实行号。

---

### 4.2 终结假修复：三相确认状态机与 Git Blob 守卫

我们建立严密的状态转移方程，剥夺大模型对“关闭缺陷”的单方面决定权。

#### 4.2.1 状态转移矩阵 (State Transition Matrix)

| 当前状态 | 本次扫描检出情况 | 物理文件 Git Blob 比较 | 连续未命中数 (`missed_count`) | 次态 (Next Status) | 触发动作与业务解释 |
| :--- | :--- | :--- | :---: | :---: | :--- |
| **`ACTIVE`** | **命中** (Tier 1~4) | 任意 | 重置为 `0` | **`ACTIVE`** | 标记为存量 `EXISTED`；更新活跃时间；用最新元数据润色详情。 |
| **`ACTIVE`** | **未命中** | **完全相同** (代码未变) | 保持当前值 | **`ACTIVE`** | **🚨 [假修复强力拦截]** 判定为大模型单次概率漏报，保持 ACTIVE，禁止变动！ |
| **`ACTIVE`** | **未命中** | **已修改** (代码物理变更) | `missed_count + 1` (< 2) | **`ACTIVE` (观察中)** | 代码发生变动，进入平滑观察期，防范单次改动引发的临时遮蔽。 |
| **`ACTIVE`** | **未命中** | **已修改** 且 触发行已删除 | `missed_count + 1` ($\ge 2$) | **`RESOLVED`** | **正式标记已修复**。代码已修改，且连续 2 轮全仓扫描确认该隐患消失。 |
| **`RESOLVED`**| **再次被检出** | 任意 | 重置为 `0` | **`REOPENED`** | **缺陷复发**。历史已被修复或关闭的物理锚点再度被检出，告警并重新激活。 |
| **任意** | 人工标记误报/忽略 | 任意 | N/A | **`SUPPRESSED`** | **人工反馈终身免疫**。历史记录记录人工标记，后续扫描命中同一锚点自动静默。 |

#### 4.2.2 核心代码实现 (`diff_engine.go`)

```go
// EvaluateDefectLifecycle 抗模型抖动平滑增量生命周期决策引擎
func EvaluateDefectLifecycle(existingRecord *models.DefectFingerprintRecord, isSeenInThisScan bool, currentFileHash string) (nextStatus string, shouldResolve bool) {
	// 1. 如果人工已标记“误报”或“不修复”，保持免疫状态
	...
	// 3. 【核心守卫】本次未被命中：首先检查物理文件内容是否发生任何变更
	isCodeModified := (currentFileHash != "" && currentFileHash != existingRecord.FileHashSnapshot)
	
	if !isCodeModified {
		// 物理代码完全没有修改！100% 为大模型单次概率漏报，绝对禁止标记 RESOLVED！
		log.Printf("[DiffGuard] Defect #%d in %s not reported by LLM, but Git Hash is unchanged (%s). Preserving ACTIVE.\n",
			existingRecord.ID, existingRecord.FilePath, currentFileHash)
		return models.DiffStatusActive, false
	}

	// 4. 物理代码确实发生了变更，启动多轮观察期平滑机制
	const MaxMissedThreshold = 2 // 连续未命中确认阈值
	newMissedCount := existingRecord.MissedCount + 1

	if newMissedCount >= MaxMissedThreshold {
		// 物理文件已改动且连续 2 轮扫描均未再检出，正式确认修复
		log.Printf("[DiffGuard] Defect #%d in %s resolved: Code modified and missed %d consecutive scans.\n",
...
│ 公式: SHA256(RepoID + TaskID + Path + Scope + PhysicalToken)│
│ 性能: 纳秒级 Hash 索引查找，覆盖 85% 正常扫描场景           │
└──────────────────────────────┬──────────────────────────────┘
                               ▼ 没命中（触发行微调或行内改写）
┌─────────────────────────────────────────────────────────────┐
│ Tier 2: 作用域单体聚类容错 (Enclosing Scope Clustering)      │
│ 条件: 同 Path + 同 Scope + 同 Category                      │
│      且 (行号位移 ≤ 15 行 或 触发行相似度 Jaccard > 0.65)   │
│ 覆盖: 12% 场景（大模型选错触发行、空指针定义点 vs 解引用点）│
└──────────────────────────────┬──────────────────────────────┘
                               ▼ 没命中（跨函数重构或大幅下移）
┌─────────────────────────────────────────────────────────────┐
│ Tier 3: Git Diff 变更块行号映射 (Hunk Tracking)              │
...
func MatchFindingInFunnel(
	...
	l1Key := fmt.Sprintf("repo:%d|task:%d|path:%s|scope:%s|token:%s",
		f.RepoID, f.TaskTypeID, anchor.NormalizedPath, anchor.EnclosingScope, anchor.PhysicalToken)
	...
	// 提取同一文件、同一函数作用域下的所有历史记录
	scopeKey := fmt.Sprintf("%s|%s", anchor.NormalizedPath, anchor.EnclosingScope)
	candidates := scopeIndex[scopeKey]

	for _, cand := range candidates {
		// 必须为相同或相近的标准分类
		if cand.Category != f.Category {
			continue
		}

		// 检查行号物理距离（是否在同一个局部逻辑块内，允许 ±15 行偏移）
		lineDiff := abs(cand.LineStart - anchor.StartLine)
		
		// 检查触发行 Token 文本相似度 (Jaccard 相似度)
		tokenSim := calculateJaccardSimilarity(cand.TriggerLine, anchor.PhysicalToken)

		if lineDiff <= 15 || tokenSim > 0.65 {
			// Tier 2 命中！这正是同一函数内的同一缺陷，只是大模型选了不同行或代码行轻微位移
			log.Printf("[Tier2 Match] Scope clustering hit: Defect #%d (line %d) matched new finding (line %d, sim=%.2f)\n",
				cand.ID, cand.LineStart, anchor.StartLine, tokenSim)
			
			// 触发自愈迁移：更新该历史记录在数据库中的 L1 指纹为最新值，无缝继承历史生命周期
...
3. **后端通用吸附器（仅 20 行通用代码，完全零业务侵入）**：
   ...
   func SanitizeCategory(rawCategory string, allowedCategories []string) string {
       ...
       return allowedCategories[len(allowedCategories)-1] // 任务约定的最后兜底分类
   }
   ```
...
为了消除《f.md》中指出的“35% 的问题文件仅在单次扫描出现”的严重抖动：
...
type DefectFingerprintRecord struct {
	...
	ScopeSymbol      string `gorm:"size:256;index" json:"scope_symbol"`           // 物理函数签名
...
| :--- | :---: | :---: | :---: |
| **代码未动时的“无故关闭”(RESOLVED)** | **高发 (绝大部分存量被误关)** | **绝对 0 起 (0%)** | **彻底消除假修复震荡 (有 Git Hash 强锁守卫)** |
...
```
Phase 1: 物理锚点与 Git Hash 守卫落地 (解决“无故关闭” - 预计 1 天)
├── 1. 修改 diff_engine.go: 引入 Git Blob Hash 守卫，物理代码未修改严禁标记 RESOLVED
├── 2. 引入 missed_count 平滑观察期，代码修改后连续 2 轮未命中方可关闭
└── 3. 验证: 代码未改动时二次全仓扫描，断言 RESOLVED 数量严格为 0

Phase 2: 指纹公式解耦与四级漏斗重构 (解决“重复报新” - 预计 1.5 天)
├── 1. 重构 fingerprint.go: L1 强指纹彻底剔除 Category，仅绑定物理源码特征
├── 2. 实现 SourceEnricher: 物理读取真实代码行并逆向回溯 AST 函数名，自愈空行号
├── 3. 升级 diff_engine.go: 实现 Tier 1 (L1强指纹) 与 Tier 2 (作用域聚类容错)
└── 4. 验证: 用现网《f.md》的 22 组重复数据做单测回归，断言对齐率 100%

Phase 3: 提示词受控与分片治理 (全面收敛 - 预计 1 天)
├── 1. tasks/*/meta.json 补充 categories 配置，统一 Hunter/Judge 的 Prompt 约束
├── 2. semantic_chunker.go 细化分片粒度至 6~8 文件，移除单片 20 候选硬截断
└── 3. 全仓双跑端到端验收与发布上线
```
