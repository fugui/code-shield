...
| **Stage 3: 上下文装配** | `task_runner.go:543~565`<br/>(`prepareOutputPaths`, `load`) | `services/runner/context.go` | 提炼为只读的 `EngineContext` 组装器 |
| **Stage 4: 静态分析引擎** | `engine_single.go`<br/>`engine_chunked.go`<br/>`engine_debate.go` | `services/engines/single/`<br/>`services/engines/chunked/`<br/>`services/engines/debate/` | 纯分析算法，统一实现 `TaskEngine` 契约，剥离 DB 依赖 |
...
| **Stage 6: 状态终结与提交**| `task_runner.go:1600~1690`<br/>(`finalize`, `markFailed`, `writeSummaryReport`) | `services/runner/finalize.go` | GORM 事务提交与指标汇总集中收口 |
...
| **微观算力调度器** | `dispatcher.go` (1,183 行) | `services/dispatcher/` | 算力池权重均衡与并发槽位租约独立自治 |
| **底层 CLI 驱动层** | `ai_cli.go`, `native_cli.go`, 各 CLI 适配器 | `services/invoker/` | 底层调用基础设施下沉，与扫描业务完全解耦 |
