...
租户的模型配置统一遵循 [07-租户分层配置与动态热加载深度设计.md](./04-租户分层配置与动态热加载深度设计.md) 定义的三层体系：
...
classDiagram
    ...
    class AIConfig {
        ...
        +TierConfig Tier1Hunter
        +TierConfig Tier2Challenger
        +TierConfig Tier3Judge
        +TierConfig Tier4Synthesis
    }
    class TierConfig {
        +string Backend
        +string Model
