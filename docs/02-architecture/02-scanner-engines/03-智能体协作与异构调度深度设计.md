...
sequenceDiagram
    ...
    Note over D,H: [Tier 1 初筛猎手] 磁盘遍历快速排查
...
flowchart TD
    subgraph MultiAgentSystem ["多智能体交叉对抗与裁决体系"]
        ...
        subgraph HunterRole ["1. Hunter Agent (初筛猎手 · Tier 1)"]
            H_Goal["目标: 追求极致召回率 (Max Recall)"]
            H_Strategy["策略: 必须 Thick Agent 遍历工作区，假设输入不可信，挖掘潜在漏洞"]
        end
        
        subgraph ChallengerRole ["2. Challenger Agent (对抗辩护 · Tier 2)"]
            C_Goal["目标: 追求极致精确率 (Min False Positive)"]
            C_Strategy["策略: 推荐 Thin LLM 极低延迟，检索 24+ 项标准事实库开展抗辩"]
        end
        
        subgraph JudgeRole ["3. Judge Agent (终审法官 · Tier 3)"]
            J_Goal["目标: 权威裁决与定级归一 (Zero Hallucination)"]
            J_Strategy["策略: 强烈推荐 旗舰推理，独立权威仲裁，基于真实源码做终判定"]
        end
...
| 智能体角色 | 核心使命 | 模型选型基准 (4 阶梯体系) | 对抗行为准则 (Behavioral Rules) |
| :--- | :--- | :--- | :--- |
| **Hunter (猎手)** | **最大化召回率 (Recall)**，挖掘所有潜在风险 | **Tier 1 初筛猎手**<br>(必须 Thick Agent: agy / opencode) | 1. 秉持“零信任”原则，假定外部入参可能为极端值。<br>2. 深度跟踪指针运算、动态扩容、缓冲区边界与异常抛出。<br>3. 允许适度激进，将可疑点全部提取为初筛案卷。 |
| **Challenger (辩护)** | **对抗性审查与剔除假阳性**，防范合谋 | **Tier 2 辩护对抗**<br>(推荐 Thin LLM: native / 备选 agy) | 1. 扮演代码作者的“诉讼律师”，全力寻找免责证据。<br>2. 严格核查：前置断言、作用域约束、编译器保障、非默认宏隔离。<br>3. **严禁合谋**：不得盲从 Hunter 的断言，必须基于代码事实反驳。 |
| **Judge (法官)** | **中立仲裁与事实裁决**，规范化定性 | **Tier 3 终审法官**<br>(强烈推荐 旗舰推理: native 旗舰 / 高阶 agy) | 1. 保持完全中立，独立分配算力与超时（默认 1800s），仅依凭代码事实裁决。<br>2. 做出判定：`CONFIRMED` (确实存在)、`REJECTED` (属于误报)、`CONDITIONAL` (需特定配置)。<br>3. 生成精准到行号的修复建议与标准化结论。 |
| **Synthesis (汇总)** | **全仓缺陷聚合与态势归因**，报告直出 | **Tier 4 全仓汇总**<br>(推荐 Thin LLM: native) | 1. 内存纯文本直传直出，消除 CLI 冷启动。<br>2. 聚合全分片确诊缺陷，生成态势评分与 Markdown/JSON 诊断报告。 |
...
### 4.1 4 阶梯异构多模型资源池与动态配比

```mermaid
flowchart LR
    subgraph ResourcePool ["4 阶梯异构模型物理资源池 (4-Tier Resource Pool)"]
        direction TB
        R_Tier1["Tier 1 资源池 (初筛 · 必须 Thick)<br>agy / opencode (10+ 并发)"]
        R_Tier2["Tier 2 资源池 (辩护 · 推荐 Thin)<br>native / 备选 agy (10+ 并发)"]
        R_Tier3["Tier 3 资源池 (终审 · 旗舰推理)<br>native 旗舰 / 高阶 agy (5 并发)"]
        R_Tier4["Tier 4 资源池 (汇总 · 推荐 Thin)<br>native 内存直出 (2 并发)"]
    end

    subgraph PipelineFlow ["任务流水线动态流转"]
        direction TB
        P_Chunk["分片源码初筛 (Hunter)"] -->|消耗 Tier 1 槽位| R_Tier1
        P_Challenger["反向质询辩护 (Challenger)"] -->|申请 Tier 2 槽位| R_Tier2
        P_Judge["终审事实裁决 (Judge)"] -->|申请 Tier 3 槽位| R_Tier3
        P_Synthesis["全仓报告综合 (Synthesis)"] -->|消耗 Tier 4 槽位| R_Tier4
    end
```
...
flowchart TD
    ...
    Check_Syntax --> ChallengeDispatch["申请 Tier 2 槽位启动 Challenger 对抗辩护"]
    ChallengeDispatch --> Check_Consensus{"Challenger 完全认同且毫无争议?"}
    
    Check_Consensus -->|无抗辩点| FastJudge["简化终审: 申请 Tier 3 槽位快速归判定案"]
    Check_Consensus -->|存在激烈争议| FullDebate["完整仲裁: 申请 Tier 3 槽位启动 Judge 深度综合裁决"]
```

1. **零候选快速放行 (Zero-Candidate Fast Pass)**：
   * 若 Tier 1 初筛未发现任何疑点，分片直接以 `PASS` 状态归档，完全不触发后续 Tier 2 辩护与 Tier 3 终审推理模型调用（**为代码仓节省 75% 以上的无谓辩论开销**）。
2. **异步管道与背压保护 (Pipeline Backpressure Queue)**：
   * Tier 1 初筛与后续辩论仲裁通过异步队列解耦，当待辩论池积压超过上限（默认 30）时自动对 Tier 1 限速，杜绝跨 Tier 资源争抢与调度死锁。
3. **单分片候选数上限限流 (Max Candidates Cap)**：
   * 单个分片若检出超过候选上限，自动聚类合并同类项，最多提取核心可疑点进入对抗辩论，防止个别极端宏展开导致辩论爆炸。
4. **辩论与裁决超时独立熔断 (Independent Timeout Circuit Breakers)**：
   * Challenger（Tier 2）单次调用时限严格控制在对应超时内（默认 1200 秒）；若超时，自动降级为保留 Hunter 原始判定并附带 `[Challenger Degraded]` 标记；
   * Judge（Tier 3）拥有独立的深思推理超时保护（默认 1800 秒），确保终审法官有足够算力做出不可逆的精准判定。
...
func (e *DebateEngine) ProcessChunk(ctx context.Context, bundle SemanticBundle, chunkIdx int) ([]models.AnalysisFinding, error) {
	...
	log.Printf("[DebateEngine] Chunk #%d: Hunter identified %d candidates. Escalating to Tier 2/3 Debate...", 
		chunkIdx, len(hunterOut.Candidates))

	// ── 步骤 2: 调度 Tier 2 (tier2_challenger) 执行 Challenger 对抗辩护 ──
	...
	// ── 步骤 3: 调度 Tier 3 (tier3_judge) 执行 Judge 终审独立裁决 ──
...
| **综合 Token 成本** | 100% 全量顶配大模型消耗 | **Tier 1 (70%) + Tier 2 (20%) + Tier 3 (10%)** | **算力成本降低 45% ~ 60%** |
