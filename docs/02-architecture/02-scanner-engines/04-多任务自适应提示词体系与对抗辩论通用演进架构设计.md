...
3. **里程碑 M3（10 大任务类型自适应验证）**：
   - 针对 `ut-effectiveness`（单测）与 `thread-create`（架构管控）进行实测，验证猎手不再漏报且辩护维度有效。
   - 验证仲裁法官输出的 `category` 100% 落在标准白名单内。
4. **里程碑 M4（多租户动态规则扩展就绪）**：
   - 与 RFC-003（租户自定义任务类型）联动，支持租户通过前端上传自定义 `analysis_prompt.md` 即可无缝享受多 Agent 辩论能力。
