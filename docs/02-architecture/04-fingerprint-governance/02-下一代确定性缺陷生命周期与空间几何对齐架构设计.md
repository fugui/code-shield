...
1. [用户痛点深层解构与旧机制的技术破产](#一-用户痛点深层解构与旧机制的技术破产)
   - 1.1 [研发团队的核心抱怨与业务影响](#11-研发团队的核心抱怨与业务影响)
   - 1.2 [为什么现有实现必然破产？（机制级根因剖析）](#12-为什么现有实现必然破产机制级根因剖析)
2. [架构演进的三大根本基石](#二-架构演进的三大根本基石-three-architectural-pillars)
   - [基石一：物理变更铁律 (Physical Code-Change Guard)](#基石一物理变更铁律-physical-code-change-guard)
   - [基石二：双轨制解耦架构 (Identity & Payload Decoupling)](#基石二双轨制解耦架构-identity--payload-decoupling)
   - [基石三：空间几何渐进漏斗 (Spatial-Geometric Matching Funnel)](#基石三空间几何渐进漏斗-spatial-geometric-matching-funnel)
3. [总体架构与端到端数据流](#三-总体架构与端到端数据流)
4. [核心技术方案深度设计](#四-核心技术方案深度设计)
   - 4.1 [物理锚点提取算法 (SourceEnricher)](#41-物理锚点提取算法-sourceenricher)
   - 4.2 [终结假修复：双层物理变更守卫与三相确认状态机](#42-终结假修复双层物理变更守卫与三相确认状态机)
   - 4.3 [终结重复报“新”：四级空间几何比对漏斗](#43-终结重复报新四级空间几何比对漏斗)
   - 4.4 [任务配置驱动分类收敛体系 (Config-Driven Taxonomy)](#44-任务配置驱动分类收敛体系-config-driven-taxonomy)
   - 4.5 [分片与大模型调度平稳化治理](#45-分片与大模型调度平稳化治理)
   - 4.6 [历史指纹无损平滑迁移方案 (Database Migration & Compatibility)](#46-历史指纹无损平滑迁移方案-database-migration--compatibility)
   - 4.7 [缺陷修复严肃分析与审计闭环机制 (Resolution Verification Engine)](#47-缺陷修复严肃分析与审计闭环机制-resolution-verification-engine)
5. [数据模型演进 (models/models.go)](#五-数据模型演进-modelsmodelsgo)
6. [架构改造预期收益与量化指标](#六-架构改造预期收益与量化指标)
7. [实施落地实施计划与验收标准](#七-实施落地实施计划与验收标准)
   - 7.1 [分期实施路线图 (5~7 天)](#71-分期实施路线图-务实工期评估5--7-天)
   - 7.2 [四大量化验收标准 (Golden Acceptance Criteria)](#72-四大量化验收标准-golden-acceptance-criteria)
...
flowchart TB
    ...
    subgraph FunnelPhase["3. 四级空间几何比对漏斗 (The 4-Tier Spatial Engine)"]
        ...
        T1 -->|❌ 未命中| T2{"Tier 2: 作用域单体聚类与独占容错\n(同 Path + 同 NormalizedScope\n且 (行号偏移 ≤ 2 行 且 Jaccard > 0.5) 或 Token Jaccard > 0.75\n或 同分类且偏移 ≤ 15 行)"}
...
| **`ACTIVE`** | **未命中** | **已修改** | **已触碰** (所在代码确实被改) | `missed_count + 1` ($\ge 2$) | **`RESOLVED`** (Low/Medium 普通缺陷) | **正式标记已修复**。普通缺陷代码已修改且连续 2 轮未检出，附带 Git Diff 证据归档。 |
| **`ACTIVE`** | **未命中** | **已修改** | **已触碰** (所在代码确实被改) | `missed_count + 1` ($\ge 2$) | **`VERIFIED_PENDING`** (Critical/High 致命漏洞) | **🚨 [高危漏洞预关闭缓冲]** 严禁直接静默关闭！流转至审计工作台由专家并排审查 Diff 确认。 |
...
func EvaluateDefectLifecycle(
	...
	if newMissedCount >= MaxMissedThreshold {
		// 物理缺陷行被实质改动，且连续 2 轮全仓扫描确认隐患消失
		// 针对 Critical/High 致命漏洞，流转为 VERIFIED_PENDING 预关闭缓冲，防范假修复放行
		if existingRecord.Severity == "CRITICAL" || existingRecord.Severity == "HIGH" ||
			existingRecord.Severity == "致命" || existingRecord.Severity == "严重" {
			log.Printf("[DiffGuard] High-severity Defect #%d (%s) code modified, moved to VERIFIED_PENDING for review.\n",
				existingRecord.ID, existingRecord.FilePath)
			return "VERIFIED_PENDING", false
		}
...
### 4.7 缺陷修复严肃分析与审计闭环机制 (Resolution Verification Engine)

> 🛡️ **架构哲学升华**：在传统的静态代码分析工具中，缺陷关闭通常只是后台静默置为 `RESOLVED`。这种**“轻率的自动关闭”**是企业级安全治理的最大盲区——既可能因大模型概率漏报把致命隐患当成已修复从而酿成生产事故（假修复放行），又使得真正的修复沦为一笔“糊涂账”（无归因、无 Commit 证据、无 Diff 对比、不知道有没有引入次生灾害）。  
> **CodeShield 必须对每一个被系统认定的“已修复”进行严肃的多维分析与证据链沉淀，使缺陷关闭成为可溯源、可解释、高公信力的资产沉淀。**

```mermaid
flowchart TD
    Candidate["历史 ACTIVE 缺陷在本次扫描中未检出\n(且所在代码行与作用域物理修改)"] --> RVE["【修复分析引擎】Resolution Verification Engine"]
    
    subgraph S1["维度一：Git 证据链物理溯源 (Proof of Fix)"]
        RVE --> GitBlame["追溯物理改动 Commit ID & 提交者 Author"]
        RVE --> HunkDiff["提取变更代码前后对比 (Before vs After Diff Hunk)"]
    end

    subgraph S2["维度二：修复模式自动分类 (Fix Pattern Taxonomy)"]
        HunkDiff --> ClassifyPattern{"修复模式智能识别"}
        ClassifyPattern --> P1["🛡️ 防御性守卫 (FIX_GUARD)\n例: 增加了 if (!ptr) return;"]
        ClassifyPattern --> P2["🗑️ 代码物理删除 (FIX_DELETE)\n例: 彻底移除了有隐患的代码行"]
        ClassifyPattern --> P3["🔄 现代化重构 (FIX_REFACTOR)\n例: 重构为 RAII 或智能指针"]
        ClassifyPattern --> P4["⚠️ 旁路修改/疑似假修 (FIX_SUSPICIOUS)\n例: 仅改动了注释、空格或无关变量"]
    end

    subgraph S3["维度三：次生风险评估与分级确认 (Graduated Verification)"]
        P1 & P2 & P3 --> GradeCheck{"严重等级 (Severity) 判定"}
        GradeCheck -->|LOW / MEDIUM 普通缺陷| AutoClose["直接确认已修复 (RESOLVED)\n附带完整 Git 证据与 Diff 详情"]
        GradeCheck -->|CRITICAL / HIGH 致命漏洞| SafeVerify["流转为【修复待确认】(VERIFIED_PENDING)\n高亮展示修复前后 Diff 对比\n研发/安全专家一键确认，或 7 天观察无复发归档"]
        
        P4 --> RejectResolve["🚨 拦截假修复！\n判定为疑似绕过，退回 ACTIVE 并附带提醒"]
    end
```

#### 4.7.1 物理证据链沉淀 (Proof of Fix)
当系统判定一个缺陷达到关闭条件时，不再只是修改一个数据库枚举，而是自动通过 Git 物理分析提取“修复事实证明 (Proof of Fix)”并写入记录：
* **`resolved_commit_hash`**：精确锁定修复该缺陷的 Git Commit SHA；
* **`resolved_author`**：记录修复该代码的开发者姓名与邮箱（明确责任与贡献归属）；
* **`resolved_diff_hunk`**：自动截取该位置修复前后的 Unified Diff 切片（例如 `+ if (ptr == nullptr) return;`）；
* **`resolved_at`**：记录精确的修复确认时间戳。

#### 4.7.2 修复模式智能识别 (Fix Pattern Taxonomy)
系统内置基于 AST 与正则的轻量规则引擎，自动对修复方式进行分类，杜绝“改了无关代码算修复”：

| 修复模式代码 | 模式名称 | 判定逻辑与代码特征 | 系统动作 |
| :--- | :--- | :--- | :---: |
| **`FIX_GUARD`** | **防御性守卫** | 在原有触发行前/外层增加了防御性判空（`if (!p)`）、边界断言（`assert`）或范围检查。 | ✅ 高度可信修复 |
| **`FIX_DELETE`** | **物理代码删除** | 包含原缺陷特征的物理行被彻底删除，且无同名未检查逻辑替代。 | ✅ 高度可信修复 |
| **`FIX_REFACTOR`**| **现代化重构** | 原裸指针替换为 `std::unique_ptr` / `std::shared_ptr`，或采用 RAII 资源管理。 | ✅ 高度可信修复 |
| **`FIX_SUSPICIOUS`**| **可疑绕过/旁路修改** | Git Diff 显示仅改动了相邻注释、排版空格、或改动了无关变量，原风险语句实质未变。 | 🚨 **拒绝关闭，强制保持 ACTIVE 并告警！** |

#### 4.7.3 严重漏洞“预关闭”缓冲机制 (`VERIFIED_PENDING`)
为了防止高危漏洞在无声无息中被误关，引入分级确认状态机：
1. **Low / Medium 普通缺陷**：
   - 物理代码修改 + 连续 2 轮全仓扫描确认消失 $\rightarrow$ 自动流转为 **`RESOLVED`**，并在报告中完整展示 Git Commit 与修复 Diff。
2. **Critical / High 致命与严重漏洞**：
   - 物理代码修改后，系统**严禁直接关闭**，而是流转为 **`VERIFIED_PENDING`（修复待确认 / 预关闭状态）**；
   - 在平台前端开设专门的 **“待确认修复 (Pending Verifications)” 审计工作台**；
   - 将【历史缺陷详情与定位】与【本次 Commit 修复前后的真实 Diff】左右并排高亮展示；
   - 研发负责人或安全专家可一键点击“通过确认”，正式归档为 `RESOLVED`；若 7 天内再次扫描无复发，系统亦可根据配置自动平滑归档。

#### 4.7.4 修复质量与次生风险质检 (Side-Effect Guard)
针对已完成修复的代码，系统可自动调用轻量规则进行次生隐患筛查：
* **锁与资源未释放**：例如修复空指针时增加了 `if (!p) return;`，但该函数在前文调用了 `mutex.lock()` 或申请了未封装的内存，导致提前 return 时引发**死锁或内存泄漏**；
* **错误码吞没**：例如为了防崩溃将异常捕获后留空（Empty Catch Block），导致上层业务静默失败；
* **质检输出**：若发现潜在次生风险，系统在生成修复凭证的同时，附带打上 `WARN_POTENTIAL_LEAK` 标签，提醒研发人员复核。
...
| **已修复问题审计与公信力** | **糊涂账 (无证据/无Diff/无归因)** | **100% 具备 Git Commit + Diff 证据链** | **高危漏洞预关闭缓冲审查，终结假修复放行** |
