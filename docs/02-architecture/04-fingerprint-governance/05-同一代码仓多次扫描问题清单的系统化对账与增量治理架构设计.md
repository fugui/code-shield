...
> **关联基线**：[01](./01-确定性源码指纹与抗抖动增量生命周期架构设计.md) / [02](./03-基于现网实测的确定性指纹演进与多任务通用分类收敛设计.md) / [03](../../04-reports/02-post-mortems/01-实测复盘-现网连续扫描重复缺陷无法识别归因分析报告.md) / [04](./02-下一代确定性缺陷生命周期与空间几何对齐架构设计.md)
...
flowchart TB
    ...
    subgraph Phase3_5["Phase 3.5: 台账退火与膨胀抑制 (Pruner)"]
        ...
        ANN -->|"低危建议 >= 2 轮 / 中危 >= 4 轮"| DORMANT["DORMANT_ARCHIVED (冷归档休眠)"]
...
- **同一性判定（主路径 vs 兜底路径）**：对账前提是 `BaseReport` 与 `CurrentReport` 属于同一 `(Repo, TaskType)`。
  1. **主路径（原生强契约）**：系统升级后产出的所有 synthesis 工件强制在头部持久化完整元数据（`meta.repo_id / meta.task_type_id / meta.report_id / meta.commit_hash`），对账引擎首选直接读取强类型元数据，实现零推导、高可靠的直接匹配；
  2. **兜底路径（存量历史工件启发式推导）**：当且仅当面对老旧存量工件（如 19371/19375，其工件内 id 字段为 0）时，启用目录/文件名正则推导作为兼容兜底：
     - `task_type` ← 报告目录名（`deep_review` → `tasks/deep-review`）；
     - `repo` ← 报告文件名中继仓名（`XY_Source`）→ 通过 `Repository` 表名称匹配；
     - `sub_campaign` ← 文件名后缀（`-sf`）→ 映射到仓库物理子目录 `sf` 作为对账根；
     - 推导失败则拒绝对账并返回人工引导交互。
...
- **R5 LLM 仲裁触发门槛与成本断路器**：
  1. **触发门槛**：仅当同文件内残差集合满足 $|A_{\text{residual}}| \ge 1$ 且 $|B_{\text{residual}}| \ge 1$，且合计条目数 $|A| + |B| \le 10$ 时触发；若残差条目超过 10 条，视为大面积重构，禁止调用大模型，直接安全降级；
  2. **Token 与并发硬约束**：使用轻量级 Thin Prompt，单次调用 Token 控制在 2K 以内；
  3. **10s 超时与安全降级**：单次 LLM 调用硬超时设置为 10 秒；若调用超时、出错或模型不可用，系统安全降级：**跳过拆并判定，A 侧未匹配条目全部标记为 `VANISHED_COVERAGE_GAP`，B 侧未匹配条目全部标记为 `NEW`**，确保端到端耗时严格受控（对齐验收项 8 < 60s 要求）。
...
| **L2 低中危退火衰减** | 低危/建议项连续 2 轮全量扫描未复现；或中危项连续 4 轮未复现（且代码未变） | 状态置为 `DORMANT_ARCHIVED`（休眠归档），移出活动台账 | 写入 `archived_items[]`，**不进 LLM Prompt，不刷屏主报告** | 判定为偶发概率噪音或边际价值极低项，冷藏降噪 |
...
> 💡 **退火轮次业务节奏校准**：在研发团队实际运作中，整仓全量扫描（`full_ledger`）通常为**周频或双周频**（每 1~2 周一次）。若设置 5 轮/10 轮将导致历史低中危边缘噪音在主台账滞留长达 2~5 个月。因此，将退火阈值精准校准为：**低危建议连续 2 轮（约 2~4 周）、中危告警连续 4 轮（约 1~2 个月）**未复现即休眠，完美匹配研发迭代周期与敏捷降噪需求。
>
> ⚠️ **退火计数铁律（排除非全量扫描干扰）**：
> `consecutive_missed_rounds` 的计数累加**严格仅计入 `full_ledger` 模式的全量扫描轮次**！当运行 `change_focus`（变更焦点模式）时，未被本次改动触碰的代码本身就处于物理旁路（Bypass）隔离状态，未命中属于必然边界而非漏报。因此，**非全量扫描对存量历史条目的退火计数器完全透明，计数器强制冻结（Freeze），绝对禁止累加**，彻底避免日常提交导致低危缺陷被错误加速休眠。

#### 3.6.3 冷归档池 `archived_items[]` 尺寸治理与 O(1) 检索
为防范数百轮持续扫描后冷池本身产生无界线性累积（导致 synthesis JSON 突破数 MB 影响 I/O 与对账性能），建立冷池治理与索引规范：
1. **已删除项保留窗口（Purge Window）**：`OBSOLETE_DELETED`（物理文件已删除项）仅在 `archived_items[]` 中保留最近 20 轮全量扫描记录；超过 20 轮未复现且物理文件未恢复的条目，由 Pruner 彻底擦除（Purge），终止无界累加；
2. **深冷转储（Deep Archive）**：对于连续处于 `DORMANT_ARCHIVED` 超过 50 轮的全量未唤醒条目，转储至外部离线归档库，从热点 `synthesis.json` 中移出；
3. **内存 O(1) 唤醒索引**：对账引擎加载 synthesis JSON 时，在内存自动构建 `archivedMap[fingerprint]*SynthesisItem` 哈希索引，确保唤醒探测始终为 $O(1)$。

#### 3.6.4 重新检出即刻无损唤醒（Seamless Resurrection on Detection）
冷归档**绝非物理删除**。当后续某一轮扫描中，大模型重新检出已处于 `DORMANT_ARCHIVED` 状态的历史缺陷时：
1. **优先查冷库**：对账漏斗在热工作集未匹配时，通过内存哈希索引秒级检索冷归档池；
...
> 1. **对账与生命周期机制完全同构**：无论全量综合还是专项，扫描范围都是整仓或全局匹配文件；未复现的内存泄漏同样是 `COVERAGE_GAP`，同样受“代码未动永不自动关闭”守卫约束，同样遵循低危连续 2 轮 / 中危连续 4 轮未复现退火休眠机制；
...
##### 3.7.2.1 存量代码向后兼容与枚举平滑迁移策略
现有代码中 [`TaskType.GovernanceMode`](file:///home/fugui/codes/code-shield/models/models.go) 数据库默认值为 `'defect_tracking'`，并在 `BeforeSave` 中包含枚举校验。为保证存量数据库、历史任务配置平滑升级且零破坏：
1. **模式路由器兜底映射（Runtime Resolver）**：
   ```go
   ...
   func resolveGovernanceMode(raw string) string {
       ...
       case "change_focus":
           return "change_focus"
       case "full_ledger", "defect_tracking", "entity_assessment", "":
           return "full_ledger" // 存量 defect_tracking 自动平滑等价为 full_ledger
       default:
           return "full_ledger"
       }
   }
   ```
2. **模型校验平滑拓宽**：`models.TaskType.BeforeSave` 校验白名单扩展为 `{"defect_tracking", "entity_assessment", "full_ledger", "change_focus"}`，存量数据无需强制刷库即可安全运行，新建任务推荐写入规范的 `full_ledger` / `change_focus`。
...
2. **“双向聚焦”判定逻辑（只报新增、顺带销账）**：
   ...
   - **向前防线（销账）**：对于历史库中原本记录在变动区内（`LineStart/LineEnd` 处于 Git Hunk 内）的 `ACTIVE` 缺陷：
     ...
     - **事务原子同步反哺（Synchronous Feedback）**：在对账产物落盘的同时，在同一数据库事务中原子更新中央库历史记录（状态变更为 `RESOLVED`，记录修复 commit 与 Proof of Fix 证据），**杜绝异步时序竞争导致的重复判定**；
...
#### 4.2.1 `item_uid` 确定性生成算法规范
为彻底摆脱脆弱数组下标（重排/过滤后失效）并避免随机 UUID 的不可记忆性，条目业务编号 `item_uid` 采用**确定性短哈希算法**：
1. **基础公式**：
   $$\text{item\_uid} = \text{fmt.Sprintf}("F\%d-\%s", \text{reportID}, \text{fingerprint}[:8])$$
   即取报告 ID 加上 64 位物理强指纹的前 8 位十六进制（例如 `F19375-12f5a8b9`）。对于跨轮次同一物理缺陷，其后半截哈希天然一致，具备极强的直观辨识度；
2. **报告内碰撞防御序列（Collision Defense）**：
   若同一报告内因多视角上报或极端哈希截断发生碰撞，通过自增序列递增：
   $$\text{item\_uid} = \text{fmt.Sprintf}("F\%d-\%s-\%d", \text{reportID}, \text{fingerprint}[:8], \text{seq})$$
3. **双层标识职责分工**：
   - `fingerprint`（64位长哈希）：机器主外键、DB 唯一索引、跨轮合并的绝对唯一事实源（SSOT）；
   - `item_uid`：面向人类展示、Markdown 报告批注、PR 评论定位的友好稳定业务别名。
...
| `item_uid` | 条目稳定业务标识符（公式：`F{report_id}-{fingerprint[:8]}`，碰撞追加 `-{seq}`），替代脆弱数组下标 |
...
| `consecutive_missed_rounds` | 连续未命中轮次计数（**仅在 `full_ledger` 全量扫描中累加**，变更焦点模式冻结） |
| `missed_count` | 物理代码已变更时的连续未命中计数（用于 L1 修复确认观察期） |
| `coverage_gap` | 布尔：本轮未复现但因代码未变而保留的状态 |
| `template_family_id` | 跨组件同模板聚簇 ID（不合并实体） |
| `recon_relation` / `recon_base_uid` | 本轮拓扑对账关系（SAME / MULTI_VIEW / SPLIT / TEMPLATE / VANISHED）与基线条目标识 |
| `severity_range` | 跨轮观测严重度极值区间（JSON 数组，如 `["建议","致命"]`），多轮评定不一致时保留极值 |
| `severity_triage` | 布尔：标记存在跨轮严重度冲突倒挂，进入人工复核队列 |
  ...
  "archived_items": [
    {
      ...
      "consecutive_missed_rounds": 2, "archive_reason": "DORMANT_ANNEALED",
      "archived_at": "2026-09-04T09:30:00+08:00",
      "note": "建议类缺陷连续 2 轮全量扫描未复现且代码未动，自动退火休眠，移出工作集",
...
1. **`report-{id}-raw-findings.json`（不可变原生检出物证）**：
   - **生成时机与责任方**：在扫描流水线 `services/task_runner.go::executeSynthesis` 方法入口处，在调用下游大模型综合与触发对账之前，**流水线的第一动作是把当前内存中的 `allFindings` 序列化并以只读权限落盘**为 `report-{id}-raw-findings.json`；
   - **内容边界**：仅忠实记录本轮大模型实际检出的原始 Findings，生成后即封存为不可变物理事实，**绝对不包含任何历史未复现补入条目**；
...
| 低危建议项连续 2 轮全量扫描未复现且代码未动 | `DORMANT_ARCHIVED` | 判定为概率噪音，退火休眠至冷归档池，不进 LLM Prompt，不刷屏主报告 |
| 中危告警项连续 4 轮全量扫描未复现且代码未动 | `DORMANT_ARCHIVED` | 退火休眠至冷归档池，不进 Prompt，保留历史索引 |
...
| **【模式 B 变更检视】变动 Hunk 覆盖的历史缺陷本次消失** | `RESOLVED_BY_CHANGE` | 本次改动成功核销既有隐患，出具 Proof of Fix 并事务原子同步反哺全量库 |
...
| 10 | 台账膨胀抑制与冷归档隔离 | ① 连续 2 轮未复现的建议类缺陷自动进入 `archived_items[]`；② `archived_items[]` 100% 隔离于 `synthesis-AI-input`；③ 物理已删除文件的缺陷流转为 `OBSOLETE_DELETED`；④ 唤醒条目 100% 标记为 `EXISTED` 继承血统，活动工作集 `items[]` 保持紧凑有界 |
...
```
Phase 1: 对账引擎骨架与流水线接入 (1 天)
├── 1. 改造 services/task_runner.go::executeSynthesis 入口：首动将 allFindings 只读落盘为 report-{id}-raw-findings.json
├── 2. 改造 synthesis 生成管线：强制写入 meta.repo_id / meta.task_type_id / meta.report_id，终结对账目录推导依赖
├── 3. 新建 services/reconciliation 包：加载两份工件，支持 resolveGovernanceMode 兼容旧版 defect_tracking
├── 4. 复用 SourceEnricher / TaxonomyNormalizer / fingerprint.go 计算物理锚点特征与 item_uid (F{id}-{fp[:8]})
└── 5. models 新增 ScanReconciliation / ReconciliationLink（收敛 Relation，增加 SeverityRange），AutoMigrate
...
├── 3. Phase 3.5 退火剪枝器：仅 full_ledger 轮次计数（change_focus 冻结）、冷池 20 轮 Purge 窗口与 O(1) 内存哈希索引
└── 4. Phase 3.7 变更焦点模式（change_focus）：变动文件物理旁路过滤器 + 变动 Hunk 事务原子同步反哺核销
...
| 顺带核销与后续全量扫描并发时序错乱 | 变更修复延迟可见，全量对账误判为漏报 | §3.7.3 顺带核销采用**数据库事务原子同步写入**中央库，完成对账即刻出具 Proof of Fix |
| R5 LLM 仲裁误配或超时拖慢整体对账 | 历史反馈错误继承或端到端耗时超标 | 严格触发门槛（合计≤10条）+ 10s 超时断路器 + 超时安全降级为独立条目，不卡死流程 |
...
| 现有代码/文件 | 本文动作 |
| :--- | :--- |
| `tasks/*/meta.json` 与 `models.TaskType` | 拓展 `governance_mode` 枚举（`full_ledger` / `change_focus`），模式路由器内置 `resolveGovernanceMode` 兼容旧版 `defect_tracking`；`BeforeSave` 校验白名单平滑扩充 |
...
| `models/models.go` | 新增 `ScanReconciliation`、`ReconciliationLink`（含 `SeverityRange`，收敛 `Relation` 为拓扑关系），AutoMigrate |
| `services/task_runner.go`（executeSynthesis 阶段） | 入口处第一时间将 `allFindings` 只读序列化为不可变 `raw-findings.json`；执行模式路由器，写回对应模式的 synthesis 台账并派生 AI 输入 |
...
## 十三、 附录：落地实现与版本归档 (Implementation & Release Archive)

### 1. 交付与归档元信息
- **归档版本**：`v1.6.0-R2R` (Report-to-Report Reconciliation & Incremental Governance)
- **归档日期**：2026-09-04
- **实施状态**：**全部完成 (Phase 1 ~ Phase 6 100% 交付上线)**
- **代码仓库分支**：`main`

### 2. 核心源码落地索引表

| 架构设计模块 | 落地源码文件 | 核心职责与实现关键点 |
| :--- | :--- | :--- |
| **数据模型与持久化** | `models/models.go`<br>`models/db.go` | 新增 `ScanReconciliation`、`ReconciliationLink`；拓展 `GovernanceMode` 包含 `full_ledger` 与 `change_focus`；`ResolveGovernanceMode` 兼容旧版 `defect_tracking`。 |
| **契约与唯一标识** | `services/reconciliation/types.go` | 定义 `SynthesisLedger`、`SynthesisMeta`、`SynthesisItem`、`ArchivedItem`；实现 UID 生成公式 `F{reportID}-{fp[:8]}` 及严重度区间合并。 |
| **物理锚点与行校准** | `services/reconciliation/anchor.go` | 代码锚点规范化清洗、物理代码行精准校准与 L1 强物理指纹生成。 |
| **六级几何漏斗** | `services/reconciliation/funnel.go` | 跨层 1:1 独占认领锁 `claimedRecordIDs`；实现 R1~R4 几何级筛选与同锚点多视角诊断合并 (`SAME_MULTI_VIEW`)。 |
| **残差仲裁与模板族** | `services/reconciliation/arbitration.go` | 实现 R5 单文件残差对齐（配置 ≤10 条、10s 超时安全降级熔断）；实现 R6 跨文件模板族聚类（`FAMILY_ID`，只聚簇不合并独立实体）。 |
| **动态退火剪枝器** | `services/reconciliation/pruner.go` | Phase 3.5 动态退火：低危 2 轮/中危 4 轮休眠，高危永不冷寂（Zero Decay）；仅 `full_ledger` 模式计数；冷池 20 轮清理与秒级无损唤醒 (`REGRESSED`)。 |
| **变更增量焦点** | `services/reconciliation/change_focus.go` | Phase 3.7 变更焦点模式：未修改文件 100% 物理旁路隔离；变动区生成 `NEW_IN_DIFF`，修复生成 `RESOLVED_BY_CHANGE` 与 Proof of Fix 凭证。 |
| **纯函数总入口** | `services/reconciliation/engine.go` | `Reconcile(*ReconcileRequest) (*ReconcileResult, error)` 纯函数总调度，涵盖 Phase 0~4 完整时序。 |
| **流水线解耦与不可变保障** | `services/task_runner.go` | `executeSynthesis` 最前置落盘不可变 `raw-findings.json`；生成 SSOT `synthesis.json` 与审计 `recon-*.json`；AI 输入设置 Top 60 / 128KB 熔断截断并注入增量概要。 |
| **消费方与双模兼容** | `services/reports/report_service.go`<br>`services/reports/exporter_reconciliation.go` | `loadAllFindingsRaw` 双模兼容解析新版台账与旧版平铺数组；支持 `Scope="reconcile"` 导出 Excel 与 JSON 对账明细。 |
| **API 路由端点** | `handlers/report_handler.go`<br>`main.go` | 注册 `GET /api/tasks/:id/report/reconciliation`，支持前端与外部调用对账审计明细。 |
| **前端治理视图与交互** | `frontend/src/components/report/ReportReconciliationTab.tsx`<br>`frontend/src/components/report/FindingCard.tsx`<br>`frontend/src/components/report/ReportSummaryTab.tsx`<br>`frontend/src/components/report/ReportViewer.tsx`<br>`frontend/src/pages/TaskTypeManagement.tsx` | 专属跨轮对账选项卡；R1~R6 漏斗吞吐条形图；认领图谱表格与筛选；全生命周期状态徽标；概览增量条；4 种治理模式管理配置。 |

### 3. 金标断言验证结果 (`reconcile_golden_test.go`)
- **回归套件路径**：`services/reconciliation/reconcile_golden_test.go`
- **11 项黄金断言全部通过**：
  1. 代码平移容差识别：`EXISTED` 命中率 100%（✅ PASS）
  2. 多视角切片去重：同位置自动合并为 `SAME_MULTI_VIEW`（✅ PASS）
  3. 跨文件模板聚类：保持独立物理实体，标记同一 `FAMILY_ID`（✅ PASS）
  4. 漏扫自动回补：历史问题再次出现打标 `GAP_FILLED`（✅ PASS）
  5. 退火剪枝休眠：低危 2 次全量未见流转为 `ARCHIVED`（✅ PASS）
  6. 高危防冷寂断言：高危漏洞永不冷寂休眠（Zero Decay）（✅ PASS）
  7. 变更焦点模式隔离：PR 模式下无关存量缺陷数为 0（✅ PASS）
  8. 变更顺带修复闭环：精准产出带 Commit 凭证的 `RESOLVED_BY_CHANGE`（✅ PASS）
  9. 并发安全断言：多协程并发对账零数据竞态（✅ PASS）
  10. 冷池定期清理：冷池达到 20 轮周期安全清理（✅ PASS）
  11. 冷池无损唤醒：归档问题后续重现标记为 `REGRESSED`（✅ PASS）
