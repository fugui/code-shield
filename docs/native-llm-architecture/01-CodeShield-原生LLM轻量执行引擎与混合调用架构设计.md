...
graph LR
    ...
    subgraph Native_Tasks [适合 Thin LLM (轻量单轮直连)]
        ...
        N4[4. Tier 1 Fast 分片毫秒级初筛]
        N5[5. Tier 3 报告汇总与态势总结]
        N6[6. 误报反馈规则与特征提炼]
    end
```
...
| **场景 4** | **分片快速初筛 (Tier 1 Fast)** | [`models/config.go:Tier1Fast`](file:///home/fugui/codes/code-shield/models/config.go#L92) | 对数百个代码分片进行快速语法/模式过滤，输出是否存在疑似风险。 | 进程并发上限受限（5~10路），扫描排队严重。 | **连接池支持 50~100 路并发初筛**，整仓扫描预处理由分钟级降至秒级。 |
...
sequenceDiagram
    ...
    Tier1-->>Task: 返回可疑候选分片 (过滤 70% 无害代码)

    Task->>Tier2_H: 针对可疑分片启动 Hunter (Thick Agent)
...
ai:
  ...
  # ── 异构模型多阶梯资源池配置 (混合编排) ──
  tiers:
    tier1_fast:                     # Tier 1 快排初筛：全面切换为 Native 高并发
      backend: "native"
      model: "glm-4-flash"
      concurrent: 50                # HTTP 连接池轻松支持 50+ 并发
      timeout_seconds: 300
