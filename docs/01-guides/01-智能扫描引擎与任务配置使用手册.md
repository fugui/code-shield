...
scanner:
  debate:
    tiers:
      # Tier 1 初筛 (Hunter 角色，负责挖掘潜在漏洞，必须具备磁盘读写能力的 Thick Agent)
      tier1_hunter:
        resource: "agy"                # 兼容单选字段
        resources: ["agy", "opencode"] # 推荐：多 Thick Agent 资源池化打散并发
        timeout_seconds: 1200          # 初筛单片超时时限 (秒)

      # Tier 2 辩护对抗 (Challenger 角色，代码内联，高吞吐防御检索)
      tier2_challenger:
        resource: "native"             # 推荐 Thin LLM 低延迟高吞吐
        resources: ["native", "agy"]   # 辩护对抗候选算力池
        timeout_seconds: 1200          # 辩护对抗单片超时时限 (秒)

      # Tier 3 终审法官 (Judge 角色，独立仲裁定性，兼听双方事实)
      tier3_judge:
        resource: "agy"                # 强烈推荐高阶旗舰推理模型
        resources: ["agy", "native"]   # 终审法官候选算力池
        timeout_seconds: 1800          # 终审裁决单片超时时限 (秒)

      # Tier 4 全仓态势汇总 (Synthesis 角色，负责报告整合排版)
      tier4_synthesis:
        resource: "native"             # 纯文本与 JSON 聚合，首选 Thin LLM 内存直出
        resources: ["native"]
