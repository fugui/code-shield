...
3. **核心约束**：
   ...
   - **行号格式**：`line_number` 必须为字符串类型 (String)，例如 `"42"` 或 `"42-45"`，严禁使用整型数字。
   - **枚举值限制**：`severity` 只能是 `致命`、`严重`、`一般`、`建议` 之一；`category` 只能是 `健壮性与稳定性`、`资源与生命周期管理`、`代码坏味道与重构问题` 之一。
   - **无问题情况**：如果未发现任何问题，`findings` 数组为空 `[]`，并在 `summary` 中说明测试代码质量良好。
