...
在分析代码前，你必须根据目标代码的编程语言，严格识别出哪些是“测试用例”，哪些是“辅助函数/工具方法”。

### 1. 属于测试用例（必须且仅针对它们在 findings 中进行审计与输出）：
* **Go**: 必须是符合 `func TestXxx(t *testing.T)` 签名的函数。
* **Java/Kotlin**: 必须是标记了 `@Test` 注解的方法。
* **Python**: 必须是以 `test_` 开头的函数，或继承自 `unittest.TestCase` 的类中以 `test_` 开头的方法。
* **C++ (GTest)**: 必须是使用 `TEST(...)`、`TEST_F(...)`、`TEST_P(...)` 等宏定义的用例。

### 2. 属于辅助函数（绝对不能当做测试用例审计，必须直接忽略）：
* 所有的初始化与清理生命周期方法（如 Go 的 `init`、Java 的 `@Before` / `@After` 系列注解方法、Python 的 `setUp` / `tearDown` 方法、GTest 的 `SetUp` / `TearDown` 成员函数等）。
* 所有的工具方法、Mock 辅助构造器、私有 helper 方法（如 `createMockUser`、`setupDB`、`assertCustomState` 等）。如果一个方法不符合上述“测试用例”的语法特征，即使它包含了断言，也只能视作辅助函数予以忽略。
...
3. **核心约束**：
   ...
   - **无遗漏扫描**：必须完整扫描整个代码文件，任何一个符合对应语言测试用例定义的函数都不得漏掉。
   - **严格过滤辅助函数**：必须首先对文件中的所有方法进行筛选，非测试用例（如 helper、setup、tearDown、fixture 类普通成员函数）一律不计入 findings 数组中，严禁对其进行审计。
